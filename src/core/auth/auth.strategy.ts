...
export class AuthStrategy extends PassportStrategy(Strategy) {
   ...
   * 此时会将解密后的 payload 作为参数传递给 validate 方法，这个方法需要做具体的授权逻辑，比如：这里是通过用户名查找用户是否存在。
   * 当用户不存在时，说明令牌有误，可能是被伪造了，此时需要抛出 UnauthorizedException 的未授权异常。
   * 当用户存在时，会讲 user 对象添加到 req 中，在之后的 req 对象中，可以使用 req.user 获取单点登录用户。
   * @param payload 用户帐号信息
   */
